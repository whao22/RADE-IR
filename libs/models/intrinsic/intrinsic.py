...
class IntrinsicPrecompute(nn.Module):
...
class IntrinsicMLP(IntrinsicPrecompute):
    def __init__(self, cfg, metadata):
    ...
    def compose_input(self, gaussians, camera):
        base_color = gaussians.get_base_color # [N, 3]
        roughness = gaussians.get_roughness # [N, 1]
        metallic = gaussians.get_metallic # [N, 1]
        
        features = torch.concatenate([base_color, roughness, metallic], dim=-1) # [N, 5]
    ...
    def forward(self, gaussians, camera):
        ...
        output = self.mlp(inp)
        intrinsic = self.intrinsic_activation(output)
        return intrinsic


def get_intrinsic(cfg, metadata):
    name = cfg.name
    model_dict = {
        "mlp": IntrinsicMLP,
    }
