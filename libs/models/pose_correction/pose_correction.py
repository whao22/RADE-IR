...
class PoseCorrection(nn.Module):
    def __init__(self, config, metadata=None):
        ...
        v_template = np.load('tools/body_models/misc/v_templates.npz')[gender]
        lbs_weights = np.load('tools/body_models/misc/skinning_weights_all.npz')[gender]
        posedirs = np.load('tools/body_models/misc/posedirs_all.npz')[gender]
        posedirs = posedirs.reshape([posedirs.shape[0] * 3, -1]).T
        shapedirs = np.load('tools/body_models/misc/shapedirs_all.npz')[gender]
        J_regressor = np.load('tools/body_models/misc/J_regressors.npz')[gender]
        kintree_table = np.load('tools/body_models/misc/kintree_table.npy')
