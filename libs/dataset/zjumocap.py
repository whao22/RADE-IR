...
from libs.utils.dataset_utils import get_02v_bone_transforms, fetchPly, storePly, AABB
from libs.scene.cameras import Camera
from libs.utils.camera_utils import freeview_camera
from libs.utils.graphics_utils import getWorld2View2, focal2fov, fov2focal
...
class ZJUMoCapDataset(Dataset):
    def __init__(self, cfg, split='train'):
        ...
        self.faces = np.load('tools/body_models/misc/faces.npz')['faces']
        self.skinning_weights = dict(np.load('tools/body_models/misc/skinning_weights_all.npz'))
        self.posedirs = dict(np.load('tools/body_models/misc/posedirs_all.npz'))
        self.J_regressor = dict(np.load('tools/body_models/misc/J_regressors.npz'))
        ...
        if self.preload:
            self.cameras = [self.getitem(idx) for idx in tqdm(range(len(self)))]
    ...
    def get_smpl_data(self):
        ...
        for idx, (frame, model_file) in enumerate(tzip(self.frames, self.model_files_list)):
