...
import torch
from radegs_rasterization import _C
...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        if raster_settings.debug:
            ...
            try:
                num_rendered, color, intrinsics, coord, mcoord, alpha, normal, depth, mdepth, radii, geomBuffer, binningBuffer, imgBuffer, output_normals = _C.rasterize_gaussians(*args)
        ...
        ctx.save_for_backward(colors_precomp, intrinsics, means3D, scales, rotations, cov3Ds_precomp, normal, radii, sh, geomBuffer, binningBuffer, imgBuffer, alpha)
        return color, intrinsics, radii, coord, mcoord, depth, mdepth, alpha, normal, output_normals

    @staticmethod
    def backward(ctx, grad_color, grad_intrinsics, grad_radii, grad_coord, grad_mcoord, grad_depth, grad_mdepth, grad_alpha, grad_normal, grad_output_normals):

        # Restore necessary values from context
        num_rendered = ctx.num_rendered
        raster_settings = ctx.raster_settings
        colors_precomp, intrinsics, means3D, scales, rotations, cov3Ds_precomp, normal, radii, sh, geomBuffer, binningBuffer, imgBuffer, alpha = ctx.saved_tensors
        ...
        if raster_settings.debug:
            cpu_args = cpu_deep_copy_tuple(args) # Copy them before they can be corrupted
            try:
                grad_means2D, grad_colors_precomp, grad_intrinsics, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_sh, grad_scales, grad_rotations = _C.rasterize_gaussians_backward(*args)
            except Exception as ex:
        ...
        assert not torch.isnan(grad_intrinsics).any()
...
class GaussianRasterizer(nn.Module):
    ...
    def forward(self, means3D, means2D, opacities, shs = None, colors_precomp = None, intrinsics_precomp = None, scales = None, rotations = None, cov3D_precomp = None):
        ...
        if intrinsics_precomp is None:
            intrinsics_precomp = torch.empty_like(means3D[..., :1])
